...
class RobotInterface:
    def __init__(self, N, H=3, dts=0.1, write_data=False):
        self.N = N
        ...
        self.cmd_vel = np.zeros([2, N]) # left and right motor rotation
        self.prev_cmd_vel = np.zeros([2, N]) + 1e-4
        ...
        if write_data:
            ...
            self.fh = open(dir_path + '/../../data/data_%d.txt' % int(time.time()), 'a')
    ...
    def print_log(self, vel=None, pilot_ids=[]):
        N = self.N
        if vel is None:
            vel = self.cmd_vel

        # format each line: N/P robot_id time pose_x pose_y pose_theta cmd_left cmd_right (v, w)
        for i in range(N):
            s = "P " if i in pilot_ids else "N "
            t = time.time()
            s += "%d %.5f " % (self.ips[i], t)
            p = ' '.join(map(str, self.pose[:, i]))
            s += p
            s += " %f %f" % tuple(vel[:, i].tolist())
            s += " %.3f %3f" % tuple(self.curr_vel[:, i].tolist())
            s += "\n"
            #  print(s)
            if self.write_data:
                self.fh.write(s)
...
class HardwareWrap:
    def __init__(self, N, pilot_ids=[]):
        self.N = N
        self.robot_int = RobotInterface(N, write_data=False)
        self.tcp_com = None
    ...
    def signal_handler(self, sig, frame):
        rospy.logwarn('Interupt with Ctrl+C!')
        self.robot_int.cmd_vel = self.robot_int.keyboard_input_each()

    def start(self):
        N = self.N
        tcp_com = self.tcp_com
        r = self.robot_int
        r.ips = tcp_com.robot_ips
        freq = 30
        count = 0
