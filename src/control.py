...
class ControlParam:
    def __init__(self, vmax=0.5,
        ...
        self.cost_Q = {
            ...
            "prev_xy": 1e-3, "prev_t": 1e-4,# final cost of connected cp
            #  "s_cp_xy": 1e2, "s_cp_t": 1,  # stage cost of connection pair
            "s_cp_xy": 1e1, "s_cp_t": 1e0,  # stage cost of connection pair
...
class Controller:
    def __init__(self, N, dt, control_param):
        ...
        # for logging
        self.data_log = {'mpc_horizon': control_param.hmpc,
                'constr_horizon': control_param.hcst,
                'num_constr': 0,
                'setup_time': 0,
                'compute_time': 0}
    ...
    def add_align_poly_constr(self, prev_cp, L):
        ...
        for cp_ids in prev_cp:
            ...
            for ti in range(1, param.hcst):
                ...
                constr_count += 1
        self.data_log['num_constr'] = constr_count
    ...
    def optimize_cp(self, cost):
        ...
        try:
            ...
            #  print("x pre:", ans.value(self.x[:, 0]))
            #  print("u pre:", ans.value(self.u[:, 0]))
            #  print("x now:", ans.value(self.x[:, 1]))
            #  print("u now:", ans.value(self.u[:, 1]))
            return np.vstack([uv, uw]).T.flatten(), ans.value(cost)
        except Exception as e:
            print(e)
            print("Solver value: ", opt.debug.value)
            opt.debug.show_infeasibilities()
        return np.zeros(2*self.N), 0
